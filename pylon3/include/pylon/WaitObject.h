...
namespace Pylon
{
    ...
    */
    ...
    is to be added when using the file descriptor as input for the select() function.
...
#   endif
    ...
    */
    ...
    public:
        ...
        WaitObject(HANDLE h, bool duplicate=true);
    ...
    protected:
        HANDLE Duplicate(HANDLE h) const;
    ...
    public:
        ...
        /** This method allows to wrap an exiting windows handle that can be used with the \c WaitForSingleObject() and
        \c WaitForMultipleObjects methods. */
        WaitObject(timer_t posixTimer, bool duplicate=true);
        ...
        GetReadyMask() method, the \c select() function can be used to wait for pylon and non-pylon events simultaneously.
        */
        int GetFd(void) const;

        timer_t GetTimer(void) const;

        /// Indicates to what kind of file descriptor set the wrapped file descriptor must be put for usage with the \c select() function.
    ...
    protected:
        type_t m_type;
        struct _WaitObjectPosix_t* m_pWaitObjectPosix;
        timer_t m_posixTimer;
#endif
    };
    ...
    */
    ...
    public:
        ...
        explicit
        WaitObjectEx(HANDLE h, bool duplicate=true);
#endif
        ...
        /// Constructor using an existing windows handle (duplicate=false -> take ownership like std:auto_ptr)
        explicit
        WaitObjectEx(timer_t posixTimer, bool duplicate=true);
#endif
    };
